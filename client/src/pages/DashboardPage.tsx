...
export function DashboardPage({ setIsAuthenticated }: { setIsAuthenticated: (auth: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      const savedGoals = localStorage.getItem('userGoals')
      console.log('Loading goals from localStorage:', savedGoals)
      if (savedGoals) {
        try {
          const parsedGoals = JSON.parse(savedGoals)
            ...
            <div key={g.id} className="goal-card">
              <div className="goal-header">
                ...
                <div className="goal-controls">
                  <div className={`status-badge status-${g.status.toLowerCase()}`}>
              ...
              {g.strategies && g.strategies.length > 0 && g.strategies.map((s, strategyIndex) => (
                <div key={strategyIndex} className="strategy-item">
                  <div className="strategy-header">
                    <h3>{s.title}</h3>
                    <div className={`status-badge status-${s.status.toLowerCase()} small`}>
                      <span className="status-icon">
                        {s.status === 'UNBEGUN' && '⏳'}
                    ...
                    <div className="action-list">
                      {s.actions.map((a, actionIndex) => (
                        <div key={actionIndex} className="action-item">
                          ...
                          <div className={`status-badge status-${a.status.toLowerCase()} small`}>
                            <span className="status-icon">
                              {a.status === 'UNBEGUN' && '⏳'}
